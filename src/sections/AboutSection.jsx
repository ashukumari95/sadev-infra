    ...
    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 xl:gap-20 items-center mb-32">
        <div className="order-2 lg:order-1">
          {/* Changed pink-500 colors to cyan-500 */}
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-500/20 border border-cyan-500/40 mb-6 backdrop-blur-md">
            <CheckCircle className="w-3 h-3 text-cyan-400" aria-hidden="true" />
            <span className="text-cyan-400 font-bold tracking-[0.2em] uppercase text-[10px]">Our Legacy</span>
          </div>
          ...
          <h2 className="text-4xl md:text-5xl xl:text-6xl font-black mb-8 leading-[1.1] text-white tracking-tighter drop-shadow-[0_5px_15px_rgba(0,0,0,0.5)]">
            Engineering the <br />
            {/* Changed purple/pink gradient hex codes to new tech-inspired blue/cyan gradient */}
            <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#06b6d4] to-[#3b82f6] drop-shadow-none">
              Next Urban Frontier
            </span>
          </h2>
          ...
          <div className="flex flex-wrap gap-4 md:gap-6">
            {/* Changed pink-500 hover/text to cyan-500 */}
            <div className="flex-1 min-w-[140px] p-6 rounded-3xl bg-black/40 border border-white/20 backdrop-blur-xl hover:border-cyan-500/50 transition-all group shadow-xl">
              <p className="text-4xl font-black text-white mb-1 group-hover:text-cyan-500 transition-colors">15+</p>
              <p className="text-[10px] text-white/70 uppercase tracking-[0.2em] font-extrabold">Global Awards for Technical Innovation</p>
            </div>
            {/* Changed purple-500 hover/text to blue-500 */}
            <div className="flex-1 min-w-[140px] p-6 rounded-3xl bg-black/40 border border-white/20 backdrop-blur-xl hover:border-blue-500/50 transition-all group shadow-xl">
              <p className="text-4xl font-black text-white mb-1 group-hover:text-blue-500 transition-colors">500+</p>
        ...
        <div className="relative order-1 lg:order-2">
          {/* Changed purple-600 blur to blue-600 blur */}
          <div className="absolute -top-10 -right-10 w-64 h-64 bg-blue-600/30 blur-[120px] rounded-full" aria-hidden="true" />
          {/* Changed pink-600 blur to cyan-600 blur */}
          <div className="absolute -bottom-10 -left-10 w-64 h-64 bg-cyan-600/30 blur-[120px] rounded-full" aria-hidden="true" />
      ...
      <div className="w-full mb-24 relative">
        <div className="text-center mb-16">
          <h3 className="text-2xl md:text-3xl font-black mb-4 text-white tracking-tight uppercase drop-shadow-md">
            {/* Changed pink-500 text to cyan-500 */}
            The <span className="text-cyan-500">Sadev Standard</span>
          </h3>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
         {values.map((v) => (
            /* Changed pink-500 hover/border to cyan-500 */
            <div key={v.title} className="p-8 xl:p-10 rounded-[2.5rem] bg-black/60 border border-white/10 hover:border-cyan-500/50 hover:bg-black/80 transition-all group relative overflow-hidden shadow-2xl">
              {/* Changed purple-to-pink gradient hover/bg to new tech-inspired blue-to-cyan */}
              <div className="w-14 h-14 rounded-2xl bg-white/10 flex items-center justify-center mb-8 group-hover:scale-110 group-hover:bg-gradient-to-br from-blue-600 to-cyan-500 transition-all duration-500 shadow-xl">
      ...
      <div className="w-full max-w-4xl mx-auto text-center px-8 py-16 mb-24 rounded-[3.5rem] bg-gradient-to-b from-white/5 to-transparent border border-white/10 shadow-2xl relative overflow-hidden">
        {/* Changed pink-500 via/transparent gradient to cyan-500 */}
        ...
        <h3 className="text-3xl md:text-4xl font-black mb-6 text-white tracking-tight drop-shadow-md">
          {/* Changed purple/pink gradient hex codes to blue/cyan gradient */}
          Redefining <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#06b6d4] to-[#3b82f6]">Asset Efficiency</span>
        </h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {testimonials.map((t) => (
            <div key={t.name} className="p-10 xl:p-14 rounded-[3.5rem] bg-black/50 backdrop-blur-md border border-white/20 relative group hover:border-white/40 transition-all shadow-2xl">
              {/* Changed pink-500 hover text to cyan-500 */}
              <Quote className="absolute top-10 right-10 w-12 h-12 text-white/10 group-hover:text-cyan-500/20 transition-colors" aria-hidden="true" />
              
              <div className="flex gap-1 mb-8">
                {[...Array(t.stars)].map((_, starIdx) => (
                  /* Changed pink-500 text/fill to cyan-500 */
                  <Star key={`${t.name}-star-${starIdx}`} className="w-4 h-4 text-cyan-500 fill-cyan-500" aria-hidden="true" />
                ))}
              </div>
              ...
              <div className="flex items-center gap-4 border-t border-white/20 pt-8">
                {/* Changed purple/pink gradient hex codes to blue/cyan gradient */}
                <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-[#06b6d4] to-[#3b82f6] flex items-center justify-center font-black text-white text-lg shadow-lg">
                  {t.name.charAt(0)}
                </div>
                <div>
                  <p className="font-black text-white text-lg tracking-tight">{t.name}</p>
                  {/* Changed pink-400 text to cyan-400 */}
                  <p className="text-[10px] text-cyan-400 font-bold uppercase tracking-widest">{t.role}</p>
                </div>
              </div>
            </div>
          ))}
